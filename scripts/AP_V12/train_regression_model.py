...
class Model():
    ...
    def __init__(self, method, scale=False, njobs=3):
        ...
        ----------
        method: str
            The type of machine learning model to train. Supported methods are 
            "poly", "lsvm", "rf", and "gtb".
        scale: bool
    ...
    def fine_tune(self, features, targets, nfolds=5, njobs=3):
        ...
        datasets = train_test_split(
            features, targets, test_size=0.25, random_state=0
        )
        ...
        self.true_and_pred = pd.DataFrame({
            'True': Ytest.values, 'Predicted': predicted
        })
    ...
    def plot_true_vs_predicted(self, out_dir, label):
        ...
        lineLB = self.true_and_pred.min().min() 
        lineUB = self.true_and_pred.max().max()
        ax.plot(
            [lineLB, lineUB], 
            [lineLB, lineUB], 
            linestyle='--', 
...
def read_data(data_file):
    ...
    features = pd.DataFrame(
        dict(zip(inputInfo['Input variable'], inputValues)), dtype=float
    )
    
    outputValues = outputInfo['Values'].split(',')
    targets = pd.Series(
        outputValues, name=outputInfo['Output variable'], dtype=float
    )
...
if __name__ == '__main__':
    ...
    for method in METHODS:
        ...
        model = Model(method, scale=SCALE)
